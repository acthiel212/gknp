...
__global__ void InitOverlapTree_1body(
    ...
    __device__ const int*   restrict ovNumAtomsInTree,
    __device__ const int*   restrict ovFirstAtom,
    __device__       int*   restrict ovAtomTreeSize,    //sizes of tree sections
    __device__       int*   restrict NIterations,
    __device__ const int*   restrict ovAtomTreePaddedSize,
    __device__ const int*   restrict ovAtomTreePointer,    //pointers to atoms in tree
    __device__ const real4* restrict posq, //atomic positions
    __device__ const float* restrict radiusParam, //atomic radius
    __device__ const float* restrict gammaParam, //gamma
    __device__ const int*   restrict ishydrogenParam, //1=hydrogen atom

    __device__       real* restrict GaussianExponent, //atomic Gaussian exponent
    __device__       real* restrict GaussianVolume, //atomic Gaussian volume
    __device__       real* restrict AtomicGamma, //atomic Gaussian gamma

    __device__       int*   restrict ovLevel, //this and below define tree
...
__device__ void InitOverlapTreeCount_cpu(
    ...
    __device__ const real* restrict global_gaussian_volume, //atomic Gaussian volume
#ifdef USE_CUTOFF
    __device__ const int* restrict tiles, __device__ const unsigned int* restrict interactionCount, __device__ const int* restrict interactingAtoms,
    unsigned int maxTiles,
    __device__ const ushort2* exclusionTiles,
#else
    unsigned int numTiles,
...
__global__ void InitOverlapTree(
    ...
    __device__ const int* restrict ovAtomTreePaddedSize, //padded allocated sizes
    __device__ const real4* restrict posq, //atomic positions
    __device__ const real* restrict global_gaussian_exponent, //atomic Gaussian exponent
    __device__ const real* restrict global_gaussian_volume, //atomic Gaussian volume
...
__device__ void reduceovCountBuffer(const int ntrees,
			 ...
			 __device__ const int*  restrict ovAtomTreePointer,    //pointers to atom trees
			 __device__       int*  restrict ovAtomTreeSize,       //actual sizes
			 __device__ const int*  restrict ovAtomTreePaddedSize, //actual sizes
			 __device__       int*  restrict ovChildrenStartIndex,
			 __device__       int*  restrict ovChildrenCount,
			 __device__       int*  restrict ovChildrenCountTop,
			 __device__       int*  restrict ovChildrenCountBottom,
  ...
  while(tree < ntrees && PanicButton[0] == 0){
	...
	if(i < tree_size){
      ...
      for(unsigned int atom_ptr = tree_ptr + local_id; atom_ptr < tree_ptr + padded_tree_size ; atom_ptr += gsize){
	ovChildrenCountTop[atom_ptr] = 0;
      }
      for(unsigned int atom_ptr = tree_ptr + local_id; atom_ptr < tree_ptr + padded_tree_size ; atom_ptr += gsize){
	ovChildrenCountBottom[atom_ptr] = 0;
...
// insertion sort
__device__ inline void sortVolumes2body(
...
__global__ void SortOverlapTree2body(
    __device__ const int* restrict ovAtomTreePointer,    //pointers to atom trees
    __device__ const int* restrict ovAtomTreeSize,       //actual sizes
    __device__ const int* restrict ovAtomTreePaddedSize, //padded allocated sizes
    __device__       int*  restrict ovLevel, //this and below define tree
    __device__       real* restrict ovVolume,
    __device__       real* restrict ovVSfp,
...
//__global__ __attribute__((reqd_work_group_size(OV_WORK_GROUP_SIZE,1,1)))
 __device__ void ComputeOverlapTree_1pass(const int ntrees,
   ...
   __device__       int* restrict NIterations,
   __device__ const int* restrict ovAtomTreePaddedSize, //padded allocated sizes
   __device__       int* restrict ovAtomTreeLock,       //tree locks
   __device__ const real4* restrict posq, //atomic positions
   __device__ const real* restrict global_gaussian_exponent, //atomic Gaussian exponent
   __device__ const real* restrict global_gaussian_volume, //atomic Gaussian prefactor
   __device__ const real* restrict global_atomic_gamma, //atomic Gaussian prefactor
   __device__       int*  restrict ovLevel, //this and below define tree
   __device__       real* restrict ovVolume,
   __device__       real* restrict ovVsp,
   __device__       real* restrict ovVSfp,
   ...
   __device__       int*  restrict ovChildrenStartIndex,
   __device__       int*  restrict ovChildrenCount,
   __device__ volatile int*   restrict ovProcessedFlag,
   __device__ volatile int*   restrict ovOKtoProcessFlag,
   ...
   __device__       real*  restrict gvol_buffer,
   __device__       unsigned int*  restrict tree_pos_buffer, // where to store in tree
   __device__       int*   restrict i_buffer,
   __device__       int*   restrict atomj_buffer,
   __device__       int*   restrict PanicButton
   ){
  ...
  __shared__ volatile real4 posq1_buffer[OV_WORK_GROUP_SIZE]; //position of "i" overlap
...
__global__ void ResetRescanOverlapTree(const int ntrees,
  __device__ const          int*   restrict ovTreePointer,
  __device__ const          int*   restrict ovAtomTreePointer,    //pointers to atom tree sections
  __device__ const          int*   restrict ovAtomTreeSize,    //pointers to atom tree sections
  __device__ const          int*   restrict ovAtomTreePaddedSize,    //pointers to atom tree sections
  __device__                int*   restrict ovProcessedFlag,
  __device__                int*   restrict ovOKtoProcessFlag
				     ){
  const unsigned int local_id = threadIdx.x;
...
__device__ void RescanOverlapTree(const int ntrees,
   ...
   __device__       int* restrict NIterations,
   __device__ const int* restrict ovAtomTreePaddedSize, //padded allocated sizes
   __device__ const int* restrict ovAtomTreeLock,       //tree locks
   __device__ const real4* restrict posq, //atomic positions
   __device__ const real* restrict global_gaussian_exponent, //atomic Gaussian exponent
   __device__ const real* restrict global_gaussian_volume, //atomic Gaussian prefactor
   __device__ const real* restrict global_atomic_gamma, //atomic gamma
   __device__ const int*  restrict ovLevel, //this and below define tree
   __device__       real* restrict ovVolume,
   __device__       real* restrict ovVsp,
   __device__       real* restrict ovVSfp,
   __device__       real* restrict ovGamma1i,
...
__global__ void InitOverlapTreeGammas_1body(
    unsigned const int             num_padded_atoms,
    unsigned const int             num_sections,
    __device__ const int*   restrict ovTreePointer,
    __device__ const int*   restrict ovNumAtomsInTree,
