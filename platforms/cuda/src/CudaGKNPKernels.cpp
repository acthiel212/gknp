...
public:
  CudaGKNPForceInfo(int requiredBuffers, const GKNPForce& force) : CudaForceInfo(requiredBuffers), force(force) {
    }
...
void CudaCalcGKNPForceKernel::CudaOverlapTree::init_tree_size(int padded_num_atoms, int tree_section_size, int num_compute_units, int pad_modulo){
...
void CudaCalcGKNPForceKernel::CudaOverlapTree::resize_tree_buffers(OpenMM::CudaContext& cu, int ov_work_group_size){
  ...
  AccumulationBuffer1_real = CudaArray::create<float>(cu, padded_num_atoms*num_sections, "AccumulationBuffer1_real");
  ...
  AccumulationBuffer2_real = CudaArray::create<float>(cu, padded_num_atoms*num_sections, "AccumulationBuffer2_real");
...
void CudaCalcGKNPForceKernel::initialize(const System& system, const GKNPForce& force) {
    verbose_level = 0;
    ...
    usePeriodic = (force.getNonbondedMethod() != GKNPForce::NoCutoff && force.getNonbondedMethod() != GKNPForce::CutoffNonPeriodic);
    useExclusions = false;
    ...
    i4_rmin = 0.;
    i4_rmax = GKNP_I4LOOKUP_MAXA;
    i4_table_size = GKNP_I4LOOKUP_NA;
    i4_lut = new GKNPI42DLookupTable(vdwrad, atom_ishydrogen, i4_table_size, i4_rmin, i4_rmax, version);
    ntypes_screener = i4_lut->ntypes_screener;
    //upload radius types
    radtypeScreened = new CudaArray(cu, cu.getPaddedNumAtoms(), sizeof(int), "radtypeParam1");
    radtypeScreener = new CudaArray(cu, cu.getPaddedNumAtoms(), sizeof(int), "radtypeParam2");
    vector<int> types(cu.getPaddedNumAtoms());
    ...
    for(int ih = 0; ih < i4_lut->tables.size() ; ih++){
      GKNPI4LookupTable *i4table = i4_lut->tables[ih];
      GKNPLookupTable *lut_table = i4table->table;
    ...
    i4YValues = CudaArray::create<float>(cu, yy2size, "i4YValues");
    i4Y2Values = CudaArray::create<float>(cu, yy2size, "i4Y2Values");
...
void CudaCalcGKNPForceKernel::executeInitKernels(ContextImpl& context, bool includeForces, bool includeEnergy) {
  ...
  maxTiles = (nb.getUseCutoff() ? nb.getInteractingTiles().getSize() : 0);
	...
	num_compute_units = nb.getNumForceThreadBlocks(); //cu.getDevice().getInfo<CL_DEVICE_MAX_COMPUTE_UNITS>();
      }else{
	...
	if(useCutoff){
      ...
      GBDerY = CudaArray::create<float>(cu, cu.getPaddedNumAtoms(), "GBDerY"); //Y intermediate variable for Born radius-dependent GB derivative
      if(GBDerBrU) delete GBDerBrU;
      GBDerBrU = CudaArray::create<float>(cu, cu.getPaddedNumAtoms(), "GBDerBrU"); //bru variable for Born radius-dependent GB derivative
      if(GBDerU) delete GBDerU;
      GBDerU = CudaArray::create<float>(cu, cu.getPaddedNumAtoms(), "GBDerU"); //W variable for self-volume-dependent GB derivative
      if(VdWDerBrW) delete VdWDerBrW;
      VdWDerBrW = CudaArray::create<float>(cu, cu.getPaddedNumAtoms(), "VdWDerBrW"); //brw variable for Born radius-dependent Van der Waals derivative
      if(VdWDerW) delete VdWDerW;
      VdWDerW = CudaArray::create<float>(cu, cu.getPaddedNumAtoms(), "VdWDerW"); //W variable for self-volume-dependent vdW derivative
	...
	if(verbose) cout << " done. " << endl;
      ...
      kernel.setArg<cl::Buffer>(index++, gtree->ovVolEnergy->getDeviceBuffer());
      kernel.setArg<cl::Buffer>(index++, grad->getDeviceBuffer());
      kernel.setArg<cl::Buffer>(index++, (useLong ? cu.getLongForceBuffer().getDeviceBuffer() : cu.getForceBuffers().getDeviceBuffer())); //master force buffer
...

    {
      ...
      defines["NUM_ATOMS"] = cu.intToString(cu.getNumAtoms());
      defines["PADDED_NUM_ATOMS"] = cu.intToString(cu.getPaddedNumAtoms());
      defines["GKNP_HB_RADIUS"] = cu.doubleToString(GKNP_HB_RADIUS);
      defines["GKNP_RADIUS_PRECISION"] = cu.intToString(GKNP_RADIUS_PRECISION) + "u";
      defines["TILE_SIZE"] = cu.intToString(CudaContext::TileSize);
      defines["FORCE_WORK_GROUP_SIZE"] = cu.intToString(ov_work_group_size);
      defines["NUM_BLOCKS"] = cu.intToString(cu.getNumAtomBlocks());
      defines["GKNP_I4LOOKUP_MAXA"] = cu.intToString(GKNP_I4LOOKUP_MAXA);
	...
	defines["USE_PERIODIC"] = "1";
      ...
      defines["CUTOFF"] = cu.doubleToString(cutoffDistance);
      defines["CUTOFF_SQUARED"] = cu.doubleToString(cutoffDistance*cutoffDistance);
      int numContexts = cu.getPlatformData().contexts.size();
      int numExclusionTiles = nb.getExclusionTiles().getSize();
	...
	program = cu.createModule(file, defines);
      ...
      testF = CudaArray::create<float>(cu, num_values, "testF");
      if(testDerF) delete testDerF;
      testDerF = CudaArray::create<float>(cu, num_values, "testDerF");
	...
	if(verbose) cout << " done. " << endl;
      ...
      kernel.setArg<int>(index++, i4_table_size);
      kernel.setArg<float>(index++, i4_rmin);
      kernel.setArg<float>(index++, i4_rmax);
      kernel.setArg<cl::Buffer>(index++, i4YValues->getDeviceBuffer());
      kernel.setArg<cl::Buffer>(index++, i4Y2Values->getDeviceBuffer());
      kernel.setArg<int>(index++, itable);
      kernel.setArg<int>(index++, num_values);
	...
	if(verbose) cout << " done. " << endl;
      ...
      kernel.setArg<unsigned int>(index++, cu.getPaddedNumAtoms()); //bufferSize
      kernel.setArg<unsigned int>(index++, num_compute_units);     //numBuffers
	...
	kernel.setArg<unsigned int>(index++, cu.getNumAtomBlocks()*(cu.getNumAtomBlocks()+1)/2);
      ...
      //radius type indexes
      kernel.setArg<int>(index++, ntypes_screener);
	...
	if(verbose) cout << " done. " << endl;
      ...
      kernel.setArg<cl::Buffer>(index++, cu.getEnergyBuffer().getDeviceBuffer()); //master energy buffer
...

    {
      ...
      defines["FORCE_WORK_GROUP_SIZE"] = cu.intToString(ov_work_group_size);
      defines["NUM_BLOCKS"] = cu.intToString(cu.getNumAtomBlocks());
      defines["GKNP_DIELECTRIC_FACTOR"] = cu.doubleToString(dielectric_factor);
...
double CudaCalcGKNPForceKernel::executeGVolSA(ContextImpl& context, bool includeForces, bool includeEnergy) {
  ...
  cu.executeKernel(resetTreeKernel, ov_work_group_size*num_compute_units, ov_work_group_size);

  if(verbose_level > 1) cout << "Executing resetBufferKernel" << endl;
  // resets either ovAtomBuffer and long energy buffer
  cu.executeKernel(resetBufferKernel, ov_work_group_size*num_compute_units, ov_work_group_size);

  if(verbose_level > 1) cout << "Executing InitOverlapTreeKernel_1body_1" << endl;
  //fills up tree with 1-body overlaps
  cu.executeKernel(InitOverlapTreeKernel_1body_1, ov_work_group_size*num_compute_units, ov_work_group_size);
  ...
  cu.executeKernel(InitOverlapTreeCountKernel, ov_work_group_size*num_compute_units, ov_work_group_size);
  ...
  cu.executeKernel(reduceovCountBufferKernel, ov_work_group_size*num_compute_units, ov_work_group_size);
  ...
  cu.executeKernel(InitOverlapTreeKernel, ov_work_group_size*num_compute_units, ov_work_group_size);

  if(verbose_level > 1) cout << "Executing resetComputeOverlapTreeKernel" << endl;
  cu.executeKernel(resetComputeOverlapTreeKernel, ov_work_group_size*num_compute_units, ov_work_group_size);

  if(verbose_level > 1) cout << "Executing ComputeOverlapTree_1passKernel" << endl;
  cu.executeKernel(ComputeOverlapTree_1passKernel, ov_work_group_size*num_compute_units, ov_work_group_size);

  //trigger non-blocking read of PanicButton, read it after next kernel below 
  cu.getQueue().enqueueReadBuffer(PanicButton->getDeviceBuffer(), CL_TRUE, 0, 2*sizeof(int), &panic_button[0], NULL, &downloadPanicButtonEvent);
  ...
  cu.executeKernel(resetSelfVolumesKernel, ov_work_group_size*num_compute_units, ov_work_group_size);
  ...
  cu.executeKernel(computeSelfVolumesKernel, ov_work_group_size*num_compute_units, ov_work_group_size);


  if(verbose_level > 1) cout << "Executing reduceSelfVolumesKernel_buffer" << endl;
  cu.executeKernel(reduceSelfVolumesKernel_buffer, ov_work_group_size*num_compute_units, ov_work_group_size);


  if(verbose_level > 1) cout << "Executing updateSelfVolumesForces" << endl;
  cu.executeKernel(updateSelfVolumesForcesKernel, ov_work_group_size*num_compute_units, ov_work_group_size);
  ...
  cu.executeKernel(InitOverlapTreeKernel_1body_2, ov_work_group_size*num_compute_units, ov_work_group_size);

  if(verbose_level > 1) cout << "Executing ResetRescanOverlapTreeKernel" << endl;
  cu.executeKernel(ResetRescanOverlapTreeKernel, ov_work_group_size*num_compute_units, ov_work_group_size);
  
  if(verbose_level > 1) cout << "Executing InitRescanOverlapTreeKernel" << endl;
  cu.executeKernel(InitRescanOverlapTreeKernel, ov_work_group_size*num_compute_units, ov_work_group_size);
  
  if(verbose_level > 1) cout << "Executing RescanOverlapTreeKernel" << endl;
  cu.executeKernel(RescanOverlapTreeKernel, ov_work_group_size*num_compute_units, ov_work_group_size);
...
void CudaCalcGKNPForceKernel::copyParametersToContext(ContextImpl& context, const GKNPForce& force) {
